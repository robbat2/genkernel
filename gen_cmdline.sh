...
longusage() {
  ...
  echo "        --build-dst=<dir>       Build dir to use (default is --build-src)"
  ...
  echo "				separate the filenames with a comma. Use full paths."
  echo "	--integrated-initramfs, --no-integrated-initramfs"
  echo "				Include/exclued the generated initramfs in the kernel"
